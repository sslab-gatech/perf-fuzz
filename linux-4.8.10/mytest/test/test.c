...
void worker() {
	...
	*(unsigned long *)mmm = 0x4242424242424242;
	...
	addr = (unsigned long)mmap((void *)0xa00000, 
								0x100000, 
...
void test1() {
	...
	mmm = mmap((void *)0xa000000, 0x110000, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_SHARED | MAP_FIXED | MAP_ANONYMOUS, 0, 0);
	printf("0x%08lx\n", (unsigned long)mmm);
	*(unsigned long *)mmm = 0x4141414141414141;

	/***** start snapshot *****/	
	snapshot_arg[0] = (unsigned long)(&cleanup);
	snapshot_arg[1] = (unsigned long)mmm;
	snapshot_arg[2] = 0x110000;
	setjmp(env);
